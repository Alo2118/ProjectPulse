...
## Modifiche Pendenti su Backend Server (192.168.52.22)

> Queste operazioni vanno eseguite sul server backend, NON sulla macchina di sviluppo.

### Da migrazioni precedenti (Phase System)
- [ ] `npx prisma migrate dev --name add_project_phase_system`
- [ ] `npx prisma db seed`
- [ ] Run migration script: `server/prisma/migrations/manual/migrate-project-phases.ts`

### Da implementazione corrente (Gap Analysis)
- [ ] Migrazione: DocumentVersion, RiskTask, User.hourlyRate, Project.budget
- [ ] Risk scale: aggiornare validazione Zod 1-5
- [ ] Risk threshold: critico ≥ 15 (era > 6)
