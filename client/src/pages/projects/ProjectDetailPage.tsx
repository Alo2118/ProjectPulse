...
export default function ProjectDetailPage() {
  ...
  // ---- Condition banners (shown above tabs) ----
  const beforeContent = p && (p.status === "on_hold" || p.status === "cancelled") ? (
  ...
  const overviewTab = p ? (
    <div className="space-y-4 pt-4">
      {/* Phase stepper (top of overview tab, matches mockup) */}
              ...
              {Object.entries(TASK_STATUS_LABELS).map(([status, label]) => {
                ...
                return (
                  <Badge
                    key={status}
                    variant="secondary"
                    className={cn("text-[10px] px-1.5 py-0 tabular-nums", STATUS_COLORS_LEGACY[status])}
                  >
                    {label} {count}
                  </Badge>
                )
