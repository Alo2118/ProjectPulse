...
import { formatDate, formatDuration, formatHoursFromDecimal } from '@utils/dateFormatters'
    ...
    <div className="space-y-5 animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          <div className="card p-5">
            <div className="flex items-center">
              <div className="p-2.5 rounded-xl bg-semantic-warning mr-3">
                <Clock className="w-5 h-5 text-semantic-warning" />
              </div>
              <div>
                <p className="meta-row-label">Ore Totali</p>
                <p className="text-xl font-bold font-mono text-semantic-warning">
          ...
          <div className="card p-5">
            <div className="flex items-center">
              <div className="p-2.5 rounded-xl bg-semantic-info mr-3">
              ...
              <div>
                ...
                <AnimatedCounter
                  ...
                  className="text-xl font-bold font-mono text-themed-primary"
                />
              </div>
            </div>
          </div>
          <div className="card p-5">
            <div className="flex items-center">
              <div className="p-2.5 rounded-xl bg-semantic-purple mr-3">
                <FolderKanban className="w-5 h-5 text-semantic-purple" />
              </div>
              <div>
          ...
          <div className="card p-5">
            <div className="flex items-center">
              <div className="p-2.5 rounded-xl bg-semantic-success mr-3">
          ...
          <div className="card p-5">
            <div className="flex items-center">
              <div className="p-2.5 rounded-xl bg-[var(--accent-primary-bg)] mr-3">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="lg:col-span-2 card">
          <div className="p-5 border-b border-[var(--accent-primary)]/15">
            <SectionHeader icon={Users} label="Per Utente" className="mb-0" />
          </div>
          <div className="p-5">
            {byUser.length === 0 ? (
              ...
              <div className="space-y-3">
                {byUser.map((user) => (
                  <div key={user.userId} className="bg-[var(--bg-hover)] rounded-lg overflow-hidden border border-transparent hover:border-[var(--accent-primary)]/10 transition-colors">
                    <button
                      onClick={() => setExpandedUser(expandedUser === user.userId ? null : user.userId)}
                      className="w-full p-3 flex items-center justify-between hover:bg-[var(--bg-hover)] transition-colors"
                    >
                      <div className="flex items-center gap-3 flex-1 min-w-0">
                        <div className="w-8 h-8 rounded-full bg-[var(--accent-primary)]/15 border border-[var(--accent-primary)]/20 flex items-center justify-center text-themed-accent font-medium text-sm">
                          {user.firstName.charAt(0)}
                          {user.lastName.charAt(0)}
                        </div>
                        <div className="flex-1 min-w-0">
                          <p className="font-medium text-themed-primary text-left text-sm">
                            {user.firstName} {user.lastName}
                          </p>
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        <span className="text-base font-bold font-mono text-semantic-warning tabular-nums">
                          {formatDuration(user.totalMinutes)}
                        </span>
                        {expandedUser === user.userId ? (
                          <ChevronUp className="w-4 h-4 text-themed-tertiary" />
                        ) : (
                          <ChevronDown className="w-4 h-4 text-themed-tertiary" />
                        )}
                      </div>
                    </button>
                    ...
                    {/* Expanded entries */}
                    {expandedUser === user.userId && (
                      <div className="border-t border-[var(--accent-primary)]/10 p-3 space-y-2 bg-[var(--bg-hover)]">
                        {getUserEntries(user.userId).slice(0, 10).map((entry) => (
                          <div
                            key={entry.id}
                            className="flex items-center justify-between text-sm p-2 bg-[var(--bg-card)] rounded-lg"
                          >
                            <div className="flex-1 min-w-0">
                              <p className="font-medium text-themed-primary truncate">
                            ...
                            <span className="text-sm font-semibold font-mono text-semantic-warning ml-2 tabular-nums">
                              {formatDuration(entry.duration)}
                            </span>
                          </div>
                        ))}
                        {getUserEntries(user.userId).length > 10 && (
                          <p className="text-xs text-themed-secondary text-center pt-2">
        ...
        <div className="card">
          <div className="p-5 border-b border-[var(--accent-primary)]/15">
            <SectionHeader icon={FolderKanban} label="Per Progetto" className="mb-0" />
          </div>
          <div className="p-5">
            {byProject.length === 0 ? (
              <div className="text-center py-8 text-themed-tertiary">
              ...
              <div className="space-y-3">
                {byProject.map((project) => (
                  <div
                    ...
                    className="p-3 bg-[var(--bg-hover)] rounded-lg border border-transparent hover:border-[var(--accent-primary)]/10 transition-colors"
                  >
                    <div className="flex items-center justify-between">
                      <div className="min-w-0 flex-1">
                        <p className="font-medium text-themed-primary truncate text-sm">
                          {project.projectName}
                        </p>
                      </div>
                      <span className="text-base font-bold font-mono text-semantic-warning ml-2 tabular-nums">
