...
import { DOMAIN_COLORS, TASK_STATUS_LABELS, STATUS_COLORS_LEGACY } from "@/lib/constants"
...
// Bar colors by task type using DOMAIN_COLORS — alpha fill with gradient-style border
function getBarStyle(task: GanttTask): React.CSSProperties {
  ...
  // Color by task type: task → cyan, subtask → teal, milestone → purple
  const domainKey =
    task.taskType === "milestone"
      ...
      ? "subtask"
      : "task"
  const dc = DOMAIN_COLORS[domainKey]
  return {
    background: dc.bg,
...
function getBarProgressStyle(task: GanttTask): React.CSSProperties {
  if (task.status === "blocked") return { background: "#ef4444" }
  const domainKey =
    task.taskType === "milestone"
      ? "milestone"
      : task.taskType === "subtask"
...
function GanttBar({ task, left, width, onClick }: GanttBarProps) {
  ...
  const progressStyle = getBarProgressStyle(task)
      ...
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            role="button"
            tabIndex={0}
            className="absolute overflow-hidden cursor-pointer transition-[filter] hover:brightness-125 focus:outline-none focus:ring-1 focus:ring-ring"
            style={{
              left,
              ...
              <div className="relative flex h-full items-center justify-between gap-1 px-2">
                <span
                  className={cn(
                    ...
                    isBlocked ? "text-red-300" : "text-white/85"
                  )}
                >
                  {task.title}
                </span>
...
function MilestoneDiamond({ left, milestone, today }: MilestoneDiamondProps) {
  ...
  const color = isOverdue ? "#ef4444" : isDone ? "#22c55e" : DOMAIN_COLORS.milestone.hex
  const glowColor = isOverdue ? "#ef444466" : isDone ? "#22c55e66" : `${DOMAIN_COLORS.milestone.hex}66`
  ...
  return (
    <TooltipProvider delayDuration={300}>
      <Tooltip>
        <TooltipTrigger asChild>
          <div
            className="absolute cursor-pointer transition-[filter] hover:brightness-130"
            style={{
              left: left - 8,
              top: 10,
              width: 16,
        ...
        >
          <div className="flex" style={{ height: "calc(100vh - 280px)", minHeight: 300 }}>
            ...
            <div
              ...
              style={{ width: LEFT_PANEL_WIDTH, background: "hsl(var(--card))" }}
            >
              {/* Left header */}
              <div
                className="flex flex-shrink-0 items-center gap-2 border-b border-border px-3"
                style={{ height: HEADER_HEIGHT, background: "hsl(var(--card))" }}
              >
              ...
              >
                {flatRows.map((row, i) => {
                  if (row.kind === "project") {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        className="flex w-full items-center gap-2 border-b border-border px-2.5 text-left transition-colors hover:bg-accent/60 select-none"
                        style={{
                          height: ROW_HEIGHT,
                      ...
                      >
                        ...
                        <FolderKanban className="h-3.5 w-3.5 flex-shrink-0" style={{ color: DOMAIN_COLORS.project.text }} />
                        ...
                        <span
                          className={cn(
                            ...
                            STATUS_COLORS_LEGACY[row.node.status] ?? "bg-muted text-muted-foreground"
                          )}
                          onClick={(e) => {
                            e.stopPropagation()
                  ...
                  if (row.kind === "milestone") {
                    ...
                    return (
                      <button
                        ...
                        type="button"
                        className="flex w-full items-center gap-1.5 border-b border-border/40 pl-5 pr-2.5 text-left transition-colors hover:bg-accent/60 select-none"
                      ...
                      >
                        ...
                        <Flag
                          ...
                          style={{ color: isOverdue ? "#ef4444" : DOMAIN_COLORS.milestone.text }}
                        />
                        <span
                          className={cn(
                  ...
                  return (
                    <div
                      key={`task-${task.id}-${i}`}
                      className="flex items-center gap-1.5 border-b border-border/30 pl-9 pr-2.5 transition-colors hover:bg-accent/40"
                      style={{ height: ROW_HEIGHT }}
                    >
                      <CheckSquare className="h-3 w-3 flex-shrink-0 text-muted-foreground/50" />
                      <button
                        type="button"
                        className="min-w-0 flex-1 truncate text-left text-[11px] text-muted-foreground hover:text-foreground transition-colors"
            ...
            <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
              ...
              >
                ...
                >
                  ...
                  <div
                    className="absolute top-0 flex w-full items-center border-b border-border/30"
                    style={{ height: 26 }}
                  >
                    {headerData.topGroups.map((grp, i) => (
                      <div
                        key={i}
                        className="absolute flex h-full items-center overflow-hidden border-r border-border/30 pl-2"
                        style={{
                    ...
                    <div
                      ...
                      style={{
                        ...
                        background: col.isToday
                          ? undefined
                          : "hsl(var(--border)/0.4)",
                        // Today column handled separately below
                        display: col.isToday ? "none" : undefined,
                      }}
                    />
                  ))}

                  {/* Today line — accent color, 2px solid */}
