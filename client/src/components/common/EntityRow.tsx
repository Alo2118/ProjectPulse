...
// --- Map entityType to ContextGradient for ProgressGradient ---

const ENTITY_TO_CONTEXT: Record<string, ContextGradient> = {
  project: "project",
  task: "task",
  risk: "danger",
  document: "indigo",
...
export function EntityRow({
  ...
  const { icons, effects, animations, getIconWrapper } = useThemeConfig()
  ...
  const iconWrapperClass = getIconWrapper(
    DOMAIN_COLORS[entityType] ?? "text-muted-foreground"
  )
  const context = ENTITY_TO_CONTEXT[entityType] ?? "project"
  ...
  return (
    <motion.div
      ...
      transition={{
        duration: effects.transitionDuration / 1000,
        type: effects.transitionType === "spring" ? "spring" : "tween",
      }}
      role="button"
      tabIndex={0}
      onKeyDown={(e) => {
    ...
    >
      {/* Domain Icon */}
      <div className={cn("shrink-0", iconWrapperClass, iconColor)}>
