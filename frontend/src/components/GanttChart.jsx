...
export default function GanttChart({ milestones, tasks, onMilestoneClick, onTaskClick }) {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'completed':
        return designTokens.colors.success.solid;
      case 'in_progress':
        return designTokens.colors.info.solid;
      case 'blocked':
        return designTokens.colors.error.solid;
      case 'waiting_clarification':
        return designTokens.colors.yellow.solid;
      default:
  ...
  const getMilestoneColor = (milestone) => {
    if (milestone.status === 'completed') return 'bg-green-600 dark:bg-green-500';
    if (milestone.status === 'cancelled') return 'bg-gray-400 dark:bg-gray-500';

    const now = new Date();
    const dueDate = milestone.due_date ? new Date(milestone.due_date) : null;
    if (dueDate && dueDate < now) return 'bg-red-600 dark:bg-red-500';
  ...
  // Mark today
  const todayOffset =
    ((new Date() - chartData.startDate) / (1000 * 60 * 60 * 24) / chartData.totalDays) * 100;

  return (
    <div className="card-lg overflow-auto">
      {/* Critical Path Info */}
      {chartData.criticalCount > 0 && (
        <div className="mb-6 flex items-start gap-3 rounded-lg border-2 border-orange-300 bg-orange-50 p-4">
          <Zap className="mt-0.5 h-5 w-5 flex-shrink-0 fill-orange-600 text-orange-600" />
          <div>
            <div className="text-sm font-bold text-orange-900">Percorso Critico Identificato</div>
            <div className="mt-1 text-xs font-medium text-orange-700">
              {chartData.criticalCount} {chartData.criticalCount === 1 ? 'task è' : 'task sono'} sul
              percorso critico. Qualsiasi ritardo in{' '}
              {chartData.criticalCount === 1 ? 'questo task' : 'questi task'} impatterà la data di
      ...
      {/* Timeline Header */}
      <div className="relative mb-6" style={{ minWidth: '800px' }}>
        <div className={`flex border-b-2 ${colors.border}`}>
          {monthHeaders.map((month, i) => (
            <div
              key={i}
              className={`border-r-2 ${colors.borderLight} py-2 text-center text-sm font-bold ${colors.text.primary}`}
              style={{
        ...
        {todayOffset >= 0 && todayOffset <= 100 && (
          <div
            className={`absolute bottom-0 top-0 z-10 w-0.5 ${designTokens.colors.error.solid}`}
            style={{ left: `${todayOffset}%` }}
          >
            <div className={`absolute -top-6 left-1/2 -translate-x-1/2 transform whitespace-nowrap text-xs font-semibold ${designTokens.colors.error.text}`}>
              Oggi
            </div>
          </div>
        )}
      ...
      <div className="space-y-2" style={{ minWidth: '800px' }}>
        ...
        {chartData.milestones.map((milestone) => (
          <div key={`milestone-${milestone.id}`} className="mb-6">
            ...
            <div className="mb-2 flex items-center">
              <div className="w-48 flex-shrink-0 pr-4">
                <button
                  ...
                  className={`text-left transition-colors hover:${designTokens.colors.info.text}`}
                >
                  <div className="flex items-center gap-2 text-sm font-bold">
                    <Calendar className={`h-4 w-4 ${designTokens.colors.info.text}`} />
                    {milestone.name}
                  </div>
                  <div className={`text-xs font-medium ${colors.text.secondary}`}>
                    {milestone.start.toLocaleDateString('it-IT')} -{' '}
                    {milestone.end.toLocaleDateString('it-IT')}
                  </div>
                </button>
              </div>
              <div className="relative h-8 flex-1">
                <div
                  className={`absolute ${getMilestoneColor(milestone)} group h-full cursor-pointer rounded-lg shadow-md transition-shadow hover:shadow-lg`}
                  style={{
                  ...
                  onClick={() => onMilestoneClick?.(milestone)}
                >
                  <div className="truncate px-2 py-1 text-xs font-medium text-white">
                    {milestone.task_count || 0} attività
                  </div>
                  {milestone.status === 'completed' && (
                    <CheckCircle className="absolute right-2 top-1/2 h-4 w-4 -translate-y-1/2 transform text-white" />
                  )}
                </div>
              </div>
            </div>
            ...
            {chartData.tasks
              ...
              .map((task) => (
                <div key={`task-${task.id}`} className="mb-1 ml-8 flex items-center">
                  <div className="w-40 flex-shrink-0 pr-4">
                    ...
                    >
                      <div className="flex items-center gap-1 truncate text-xs font-semibold">
                        {task.isCritical && (
                          <Zap className="h-3 w-3 fill-orange-600 text-orange-600 dark:fill-orange-500 dark:text-orange-500" />
                        )}
                        {task.title}
                      </div>
                      {task.hours > 0 && (
                        <div className={`text-xs font-medium ${colors.text.secondary}`}>{task.hours}h</div>
                      )}
                    </button>
                  </div>
                  <div className="relative h-6 flex-1">
                    <div
                      className={`absolute ${getStatusColor(task.status)} h-full cursor-pointer rounded shadow transition-shadow hover:shadow-md ${task.isCritical ? 'ring-2 ring-orange-500 dark:ring-orange-400 ring-offset-1' : ''}`}
                      style={{
                      ...
                      title={
                        task.isCritical
                          ? '⚡ Task sul percorso critico - qualsiasi ritardo impatta la consegna del progetto'
                          : ''
                      }
                    >
                      ...
                      {task.progress > 0 && (
                        <div
                          className="absolute bottom-0 left-0 top-0 rounded-l bg-white bg-opacity-30"
                          style={{ width: `${task.progress}%` }}
                        ></div>
                      )}
                      <div className="relative flex items-center justify-between truncate px-2 py-1 text-xs text-white">
        ...
        {chartData.tasks.filter((task) => !task.milestone_id).length > 0 && (
          <div className={`mt-6 border-t-2 ${colors.border} pt-6`}>
            <div className={`mb-3 flex items-center gap-2 text-sm font-bold ${colors.text.primary}`}>
            ...
            {chartData.tasks
              ...
              .map((task) => (
                <div key={`task-${task.id}`} className="mb-2 flex items-center">
                  <div className="w-48 flex-shrink-0 pr-4">
                    <button
                      onClick={() => onTaskClick?.(task)}
                      className="text-left transition-colors hover:text-blue-600 dark:hover:text-blue-400"
        ...
        {chartData.milestones.length === 0 && chartData.tasks.length === 0 && (
          <div className={`py-12 text-center ${colors.text.secondary}`}>
            <AlertCircle className={`mx-auto mb-3 h-12 w-12 ${colors.text.tertiary}`} />
            <p className="font-semibold">Nessuna milestone o attività da visualizzare</p>
            <p className="mt-2 text-sm">Crea milestone e attività con date per vederle nel Gantt</p>
          </div>
        )}
      </div>

      {/* Legend */}
      <div className={`mt-8 border-t-2 ${colors.border} pt-6`}>
        <div className="mb-3 flex flex-wrap gap-4 text-xs font-semibold">
          <div className="flex items-center gap-2">
            <div className="h-4 w-4 rounded bg-gray-400 dark:bg-gray-500"></div>
            <span>Da fare</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`h-4 w-4 rounded ${designTokens.colors.info.solid}`}></div>
            <span>In corso</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`h-4 w-4 rounded ${designTokens.colors.yellow.solid}`}></div>
            <span>In attesa</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`h-4 w-4 rounded ${designTokens.colors.error.solid}`}></div>
            <span>Bloccato</span>
          </div>
          <div className="flex items-center gap-2">
            <div className={`h-4 w-4 rounded ${designTokens.colors.success.solid}`}></div>
            <span>Completato</span>
          </div>
        </div>
        <div className="flex items-center gap-2 text-xs font-semibold">
          <div className={`h-4 w-4 rounded ${designTokens.colors.info.solid} ring-2 ring-orange-600 dark:ring-orange-500`}></div>
          <Zap className="h-3 w-3 fill-orange-600 text-orange-600 dark:fill-orange-500 dark:text-orange-500" />
          <span className="font-bold text-orange-800 dark:text-orange-400">Percorso Critico</span>
          <span className={colors.text.secondary}>
            - Task che impattano direttamente la data di consegna
          </span>
        </div>
      </div>
    </div>
